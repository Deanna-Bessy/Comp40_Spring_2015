...
operation_info static inline parse_operator(instruction op_code)
{
	...
	unsigned hi = OP_LSB + 4; /* one beyond the most significant bit */
        assert(hi <= 64);
        operation_info parsed_info;

        int operator = shr(shl(op_code, 64 - hi), 60);

        parsed_info.operation = operator;

        if (operator == 13) {
                parsed_info.value = shr(shl(op_code, 64 - (VAL_LSB+VAL_WIDTH)), 64 - VAL_WIDTH);
                parsed_info.A = shr(shl(op_code, 64 - (REG_A_OP_13+ 3)), 61);
        }
        else {
                parsed_info.A = shr(shl(op_code, 64 - (REG_A_LSB+ 3)), 61);
                parsed_info.B = shr(shl(op_code, 64 - (REG_B_LSB+ 3)), 61);
                parsed_info.C = shr(shl(op_code, 64 - (REG_C_LSB+ 3)), 61);
        }
...
void run(const char *fp)
{
	 ...
	 * instruction until HALT is reached. 
         */
        while (parsed_op_code.operation != 7 && program_counter < memory->segments[0].length) {
